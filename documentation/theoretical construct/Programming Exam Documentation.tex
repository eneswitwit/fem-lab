...
We are going to use this heuristic rule to compute $ | u_{h} - u_{h/2} | $, which we will view as a certain a posteriori error indicator. Unfortunatley it will not have the property modern a posteriori error indicators have, which is to be a upper bound. Still with empirical investigations it seems that our error indicator is quite well.
